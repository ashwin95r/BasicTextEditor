...
static char *tips[][2] =
  {
    ...
    {"puts", "(const char *s)"},
        ...
        /*        auto break case char const continue default do double else enum extern float goto if int long register return short signed sizeof static struct switch typedef union unsigned void volatile         
        */};
...
void font_highlight ( GtkWidget *text_view, GtkTextIter *arg1)
{
  ...
  if (found == 0)
    return;
        ...
        if(GTK_IS_TEXT_TAG_TABLE (table1))
                gtk_text_tag_table_remove(table1,highlight_tag2);

        highlight_tag2 = gtk_text_buffer_create_tag(buffer, NULL,"foreground", "red", NULL);        
        
        if(GTK_IS_TEXT_TAG_TABLE (table1))
                gtk_text_tag_table_add(table1, highlight_tag2);

  if (found){
      /* If found, highlight the text. */
        gtk_text_buffer_apply_tag( buffer,highlight_tag2,&mstart,&mend );        
        }
...
static void fonta(GtkToolButton *cut, TextEditor *editor);
void select_font(GtkToolButton *cut, TextEditor *editor);
...
int main (int argc, char *argv[])
{
  ...
  font3 = gtk_tool_button_new_from_stock(GTK_STOCK_SELECT_FONT);
  gtk_toolbar_insert(GTK_TOOLBAR(toolbar), font3, -1);
  g_signal_connect (G_OBJECT (font3), "clicked",
                    G_CALLBACK (fonta), 
                    (gpointer) (editor));
...
/*    font2 = gtk_tool_button_new_from_stock(GTK_STOCK_SELECT_COLOR);
    gtk_toolbar_insert(GTK_TOOLBAR(toolbar), font2, -1);
    g_signal_connect(G_OBJECT(font2), "clicked", G_CALLBACK(select_font), (gpointer) (editor));*/
...
void select_font(GtkToolButton *cut, TextEditor *editor)
{
  ...
  if (result == GTK_RESPONSE_OK || result == GTK_RESPONSE_APPLY)
  {
    ...
    gtk_widget_modify_fg(GTK_WIDGET(editor->textview),
                   GTK_STATE_NORMAL,
                   &color);
  } 
...
*/
//font style
static void fonta(GtkToolButton *cut, 
             TextEditor *editor)
{
	GtkResponseType result;
    ...
    if (result == GTK_RESPONSE_OK || result == GTK_RESPONSE_APPLY)
    {
    	...
    	gtk_widget_modify_font(GTK_WIDGET(editor->textview), font_desc);
